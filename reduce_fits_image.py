...
parser.add_argument('-e', dest='fits_extension', default=0, type=int,
...
if os.path.exists('bias.fits'):
    ...
    if os.path.exists('../bias.fits'):
        ...
        # try:
            biaslist = all_files.files_filtered(imagetyp='bias')

            if len(biaslist)==0:
                biaslist = all_files.files_filtered(imagetyp='zero')
            if len(biaslist)==0:
                biaslist = all_files.files_filtered(object='bias')
            if len(biaslist)==0:
                biaslist = all_files.files_filtered(exptype='Bias')
...
for image in files_to_red:
    ...
    if len(flatlist)==0:
        flatlist = all_files.files_filtered(object='flat', filter=filt)
    if len(flatlist)==0:
        flatlist = all_files.files_filtered(object='dome flat', filter=filt)
    if len(flatlist)==0:
        flatlist = all_files.files_filtered(object='dome', filter=filt)
    if len(flatlist)==0:
        flatlist = all_files.files_filtered(object='sky flat', filter=filt)
    if len(flatlist)==0:
        flatlist = all_files.files_filtered(exptype='flat', filter=filt)
    ...
    try:
        ...
        try:
            obs = ccdproc.trim_image(obs, fits_section=obs.header['DATASEC'] )
        except:
            pass
