...
########### Centering
    ...
    if filtername not in im_info:
        im_info[filtername] = {}
        im_info[filtername]['refim'] = image
        im_info[filtername]['refx'] = SNco[0] + del_x
        im_info[filtername]['refy'] = SNco[1] + del_y
        im_info[filtername]['refco'] = cent[:,1:3]

    im_info[filtername][image] = {'dx': 0.0, 'dy': 0.0, 'mjd': 0.0}

    im_info[filtername][image]['dx'] = im_info[filtername]['refx'] - (SNco[0] + del_x)
    im_info[filtername][image]['dy'] = im_info[filtername]['refy'] - (SNco[1] + del_y)

    im_info[filtername][image]['mjd'] = mjd
...
for i in im_info:
    np.savetxt('refcoords.txt',im_info[i]['refco'],fmt='%4.3f')

    refim = im_info[i]['refim']

    listfile = open('imagelist.txt','w')

    comfile = open('comlist.txt','w')

    shiftfile = open('shifts.txt','w')
    ...
    iraf.imalign(input='@imagelist.txt',reference=refim,coords='refcoords.txt',output='shifted_//@imagelist.txt',shifts='shifts.txt')

    iraf.imcombine(input='@comlist.txt',output='stack_'+refim,combine='median',offsets='none')
